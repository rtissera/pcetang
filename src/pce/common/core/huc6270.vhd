...
-- this lineage). Every other file in the engine donor is used unmodified, straight from
-- upstream/ -- this is the ONE file this port changes, in exactly three places (search
-- "GOWIN FIX" below for all three):
--
...
-- slack for a same-port read-then-delayed-write with zero interaction with port A's
-- tile-fetch writes, no arbitration, no second memory, and no change to the consumption
-- pipeline's own pre-existing one-cycle read latency.
--
-- 2. A donor reset aggregate assigns a 2-bit literal "00" to PAL, a 4-bit record field.
-- GHDL's strict --std=93 rejects this outright ("element is out of the bounds"), not
-- just a style warning. Zero-extended to "0000" -- same runtime value on this reset path
-- (both mean PAL=0), not a behavior change. Confirmed present identically in the
-- pristine donor file under the same GHDL flags before this fix, so it's a real
-- pre-existing donor quirk, not something introduced by fix 1 above.
--
-- 3. Even with fix 1's retiming applied (or without it -- confirmed present in the
-- unmodified donor's original coincident wiring too), GW5A's automatic dpram inference
-- (src/common/mem/bram_gowin.vhd) produces WRITE_MODE 2'b10 for any memory shaped like
-- "two independently-addressed writers, one side also reads" and gets rejected at
-- place-and-route -- ERROR (PA2122). This affects BOTH SPR_LINE_BUF0/1 (fixed via
-- src/common/mem/dpram9_dpb_wm01.vhd) AND SAT (same shape: DMA write on port A,
-- eval-read/clear-write mux on port B -- fixed via
-- src/common/mem/dpram8x16_dpb_wm01.vhd), discovered one at a time because Gowin's
-- error only reports the first instance it finds; fixing SPR_LINE_BUF0/1 exposed SAT
-- hitting the identical wall. Root cause: Gowin's own DPB/DPX9B primitive documentation
-- (UG285) states same-address cross-port read+write isn't supported, and neither
...
--
-- Everything else in this file is byte-identical to the donor.
...
architecture rtl of HUC6270 is
	...
	-- GOWIN FIX: one-raw-cycle-delayed clear-write pulse (per buffer, since buffer
	-- selection alternates by SPR_OUT_X(0) parity, only one is ever active at a time)
	-- and its address, shared by both buffers -- see this file's header comment.
	signal SPR_LINE_CLR_WR   : std_logic_vector(1 downto 0) := (others => '0');
	signal SPR_LINE_CLR_ADDR : std_logic_vector(8 downto 0) := (others => '0');
	-- GOWIN FIX: port B's actual (muxed) address per buffer -- VHDL-93 port maps can't
	-- take a conditional expression directly, so this is computed by a concurrent signal
	-- assignment below and fed into the instantiations as a plain signal.
...
begin
	...
	-- GOWIN FIX: same two-independently-addressed-writers shape and same ERROR (PA2122)
	-- as SPR_LINE_BUF0/1 -- see this file's header (fix 3) and
	-- src/common/mem/dpram8x16_dpb_wm01.vhd's own header.
	SAT : entity work.dpram8x16_dpb_wm01
	port map(
		clock		=> CLK,
	...
	begin
		...
		if RST_N = '0' then
			...
			SPR_FIND <= '0';
			-- GOWIN FIX: donor has "00" here (2 bits) against PAL's 4-bit field -- a
			-- latent width bug GHDL's strict --std=93 rejects outright ("element is out
			-- of the bounds", not just a style warning). Zero-extended to "0000": same
			-- runtime value (both mean PAL=0) on this reset path, not a behavior change.
	...
	begin
		if RST_N = '0' then
			SPR_LINE_CLR_WR <= (others => '0');
		elsif rising_edge(CLK) then
			SPR_LINE_CLR_WR(0) <= SPR_LINE_CLR and DCK_CE and not SPR_OUT_X(0);
			SPR_LINE_CLR_WR(1) <= SPR_LINE_CLR and DCK_CE and SPR_OUT_X(0);
			SPR_LINE_CLR_ADDR  <= std_logic_vector(SPR_OUT_X(9 downto 1));
		end if;
	end process;

	-- GOWIN FIX: address_b tracks the live read address except for the one cycle
	-- following a clear request, when it switches to the delayed address so the
	-- clear-write commits a cycle after (never during) that address's own read.
	SPR_LINE_ADDR_B0 <= SPR_LINE_CLR_ADDR when SPR_LINE_CLR_WR(0) = '1' else std_logic_vector(SPR_OUT_X(9 downto 1));
	SPR_LINE_ADDR_B1 <= SPR_LINE_CLR_ADDR when SPR_LINE_CLR_WR(1) = '1' else std_logic_vector(SPR_OUT_X(9 downto 1));

	-- GOWIN FIX: work.dpram (automatic inference) reliably produces WRITE_MODE 2'b01 for
	-- every OTHER memory in this design, but fails on GW5A specifically for this buffer's
	-- two-independent-writers shape -- ERROR (PA2122), confirmed present even with the
	-- unmodified donor's original (non-retimed) wiring. Root cause: Gowin's own DPB
	...
	port map(
		...
		address_b=> SPR_LINE_ADDR_B0,
		wren_b	=> SPR_LINE_CLR_WR(0),
		q_b		=> SPR_LINE_Q(0)
	);

	SPR_LINE_BUF1 : entity work.dpram9_dpb_wm01
	port map(
		clock		=> CLK,
		...
		address_b=> SPR_LINE_ADDR_B1,
		wren_b	=> SPR_LINE_CLR_WR(1),
		q_b		=> SPR_LINE_Q(1)
	);
