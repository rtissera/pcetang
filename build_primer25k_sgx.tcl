...
# pcetang Primer 25K SGX: SuperGrafx (dual-VDC), TangCore-integrated (iosys_bl616),
# HuCard-only, EXT_VRAM0+EXT_VRAM1 (both VDCs' VRAM on the real Tang SDRAM V1.3 PMOD --
# VRAM0 via port A, VRAM1 via port C's new real 16-bit+line-refill additions). See
# pcetang_primer25k_sgx.vhd's own header for the real BSRAM/contention feasibility
# basis. Run from the repo root: gw_sh build_primer25k_sgx.tcl
...
# LITE=>0 on this board (SGX enabled) activates generate_CHEAT.GAMEGENIE in
# pce_top.vhd, which build_primer25k.tcl's own file list never needed (LITE=>1
# there). Missing this caused a real PA2050 "Module 'CODES' is instantiated but
# not defined" error on this board's first probe build.
...
add_file src/pcetang_primer25k_sgx.vhd
add_file src/pcetang_primer25k.cst
add_file src/pcetang_primer25k.sdc

set_option -synthesis_tool gowinsynthesis
set_option -output_base_name pcetang_primer25k_sgx
set_option -verilog_std sysv2017
set_option -vhdl_std vhd2008
set_option -top_module pcetang_primer25k_sgx
...
# Alternate PnR algorithm (see build_primer25k.tcl's own comment) -- applied from the
# start here, not retrofitted after a margin regression.
